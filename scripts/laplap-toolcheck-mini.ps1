...
function Get-DiskHealthReport {
    ...
    # 1) Thu Get-StorageReliabilityCounter (SSD NVMe/SATA co Wear; mot so HDD driver cung co PowerOnHours)
    $rc = $null
    if ($PhysicalDisk) {
    ...
    if ($rc -and $rc.Wear -ne $null -and $rc.Wear -ge 0) {
        # Wear = 0 -> moi 100%; Wear tang -> suc khoe giam.
    ...
    # SSD neu co source WMI reliability counter thi tin cay.
    if ($Kind -like "*SSD*") {
    ...
    # 2) HDD: doc WMI SMART attributes de suy ra trang thai that.
    #    MSStorageDriver_ATAPISmartData: thuoc tinh ID 5 = ReallocatedSectorsCount,
    #    ID 197 = CurrentPendingSector, ID 199 = UDMA CRC Error.
    $predictFailure = $null
    try {
    ...
    try {
        ...
        foreach ($sb in $smartBlocks) {
            ...
            # VendorSpecific: 2 byte header, 12 byte SMART attribute struct x 30.
            # Moi attribute: ID (1), Status (2), Value (1), Worst (1), Raw (4).
    ...
    # Quy tac suc khoe HDD:
    #  - Reallocated > 10: Critical
    #  - Pending > 0 hoac Reallocated > 0: Warning
    #  - PredictFailure = True: Warning
    #  - Nguoc lai: Healthy
...
function Invoke-SystemScan {
    ...
    foreach ($d in $disks) {
        ...
        # Trang thai cuoi cung: uu tien SMART, fallback HealthStatus cua PhysicalDisk / DiskDrive.
...
# Neu nguoi dung da co Toolcheck/ canh scanner (tai lan truoc) thi mo them menu,
# nguoc lai chi can quet xong la thoat.
function Open-Tool {
